...
const indexPairs = [
  ...
  ['Remova grátis objetos ou marcas-d’água com a IA LaMa e depois retire o fundo se precisar. Visitantes e usuários podem limpar até 50 imagens por lote.', '免费使用 LaMa AI 去除物体或类似水印的痕迹，再按需使用 AI 去背景。游客和登录用户每批都可清除最多 50 张图片。'],
  ...
  ['Não é possível excluir imagens durante o processamento.', '处理图片时不能删除。'],
  ['Não foi possível excluir esta imagem do espaço de trabalho salvo. Seu trabalho atual continua aberto.', '无法从已保存的工作区删除这张图片，当前工作仍保持打开。'],
  ['Limpeza local de objetos', '本地去除物体'],
  ['Limpeza precisa de objetos com IA', 'AI 精细去物体 / 水印'],
  ['Limpeza LaMa grátis para objetos e marcas-d’água', '免费使用 LaMa 修复物体和类似水印的痕迹'],
  ...
  ['Local · Grátis', '本地 · 免费'],
  ['IA · Grátis · Sem créditos', 'AI · 免费 · 不扣积分'],
  ['Uma imagem derivada e a máscara são enviadas temporariamente ao armazenamento privado, apagadas após a recuperação no navegador e mantidas por no máximo 24 horas.', '原图派生文件和蒙版会临时上传到私有存储；写回浏览器后立即删除，异常时最长保留 24 小时。'],
  ...
  ['A remoção de fundo custa 1 crédito somente após sucesso. A limpeza de objetos com IA, ajustes de fundo e exportação continuam grátis.', '每张图片仅在 AI 去背景成功后扣除 1 积分。AI 去物体 / 水印、背景调整和导出均免费。'],
  ...
  ['Seu espaço fica neste navegador por até 24 horas. A limpeza com IA envia temporariamente uma imagem derivada e a máscara ao armazenamento privado; a remoção de fundo envia a imagem necessária à API.', '工作区会在当前浏览器中保存最多 24 小时。AI 去物体 / 水印会临时上传原图派生文件和蒙版到私有存储；AI 去背景会把所需图片发送到对应处理 API。'],
  ...
  ['Pronto', '就绪'],
  ['Limpeza com IA salva. Apenas esta imagem precisa ser preparada novamente.', 'AI 清除结果已保存，仅需重新处理当前图片。'],
  ['Remover fundo', '去除背景'],
  ['Baixar ZIP', '下载 ZIP'],
...
const privacyPairs = [
  ...
  ['Última atualização: 29 de julho de 2026', '最后更新：2026 年 7 月 29 日'],
  ...
  ['Também usamos análise própria e com coleta mínima para entender se os visitantes selecionam imagens, iniciam uma ferramenta, obtêm um resultado e fazem o download. Guardamos apenas identificadores de visitante e sessão codificados no servidor, além de dados gerais como categoria do dispositivo, idioma, código do país, domínio de referência, faixa de quantidade de arquivos, duração e código de erro padronizado. Quando há login, um hash irreversível da conta permite que o relatório administrativo protegido diferencie usuários com e sem compra concluída; os eventos não contêm e-mail nem ID bruto do usuário, e o relatório mostra apenas a conta mascarada. A base de análise não armazena imagens, nomes de arquivos, máscaras, chaves de armazenamento, IPs completos, impressões digitais completas do navegador ou URLs de referência completas. Global Privacy Control e Do Not Track são respeitados.', '我们还使用隐私最小化的第一方产品统计，了解访客是否选择图片、开始使用工具、得到结果和完成下载。事件只保存经过服务器加密的访客及会话标识，以及设备大类、语言、国家代码、来源网站主机名、文件数量区间、处理耗时和标准化错误代码等粗粒度信息。登录状态下会额外保存不可逆账户哈希，让受管理员鉴权保护的报表区分登录未充值用户和已充值用户；运营事件不保存邮箱或明文用户 ID，报表中的账号也只显示脱敏结果。运营统计数据库不保存图片、文件名、蒙版、对象存储地址、完整 IP、完整浏览器指纹或完整来源网址。浏览器开启 Global Privacy Control 或 Do Not Track 时不会发送运营统计。'],
  ...
  ['Ao usar a limpeza precisa de objetos ou marcas-d’água com IA, uma imagem derivada e sua máscara são enviadas temporariamente ao armazenamento privado do ShopBG Remover e processadas pelo nosso serviço LaMa auto-hospedado. As entradas são apagadas no estado final. O resultado é excluído após a recuperação no navegador; se ela for interrompida, permanece por no máximo 24 horas. A remoção de fundo envia a imagem necessária à API. O navegador pode guardar por até 24 horas o original, a fonte editada, o recorte IA, a saída, o estado e as configurações no IndexedDB.', '使用 AI 精细去物体 / 水印时，原图派生文件和蒙版会临时上传到 ShopBG Remover 私有存储，并由我们自托管的 LaMa 服务处理。输入文件在任务进入最终状态后删除；结果写回浏览器后立即删除，如恢复中断则最长保留 24 小时。AI 去背景会把所需图片发送到对应处理 API。为支持刷新恢复，浏览器可能在 IndexedDB 中保存原图、已编辑源图、AI 前景、输出、任务状态和合成设置，最长 24 小时。你可以随时在工作区清除这些数据。'],
  ...
  ['Retemos sua conta e registros de uso enquanto sua conta estiver ativa. Os eventos de análise própria são mantidos por no máximo 180 dias. Você pode solicitar a exclusão de sua conta e dados associados a qualquer momento enviando um e-mail para <a href="mailto:yubudong2023@gmail.com">yubudong2023@gmail.com</a>. Processaremos solicitações de exclusão em 30 dias.', '账户有效期间，我们会保留账户和使用记录。第一方运营事件最长保留 180 天。你可以随时发送邮件至 <a href="mailto:yubudong2023@gmail.com">yubudong2023@gmail.com</a> 申请删除账户及相关数据，我们会在 30 天内处理。'],
...
const termsPairs = [
  ...
  ['<strong>Cota grátis:</strong> Visitantes podem concluir 3 remoções de fundo com IA no total. Usuários cadastrados recebem até 10 remoções de fundo grátis vitalícias, incluindo o uso anterior como visitante. Créditos grátis expiram após 30 dias. A limpeza precisa de objetos e marcas-d’água com IA é grátis para visitantes e usuários, até 50 imagens por lote; visitantes podem limpar até 50 imagens por dia.', '<strong>免费额度：</strong>游客累计可完成 3 次 AI 去背景。注册用户终身累计最多可获得 10 次免费 AI 去背景额度，其中包含此前的游客使用次数；注册免费积分 30 天后过期。AI 精细去物体 / 水印对游客和登录用户均免费，每批最多 50 张；游客每天最多清除 50 张。'],
  ...
  ['<strong>Cobrança:</strong> Um crédito só é debitado após uma remoção de fundo bem-sucedida. Limpeza de objetos com IA, edição no navegador, troca de fundo, downloads, novas exportações e ZIP são grátis.', '<strong>扣费：</strong>每张图片仅在 AI 去背景成功后扣除 1 积分。AI 去物体 / 水印、浏览器编辑、更换背景、下载、重新导出和创建 ZIP 均免费。'],
...
function addChineseXianyuPurchase(source) {
  ...
  return source.replace(
    '</body>',
    '<script src="/xianyu-purchase.js?v=20260729-admin-operations-v1"></script>\n</body>',
  );
}
